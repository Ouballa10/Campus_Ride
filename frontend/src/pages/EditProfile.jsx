...
function normalizeVehiclePhotos(value) {
  ...
  if (typeof value === "string" && value.trim()) {
    try {
      ...
      if (Array.isArray(parsedValue)) return normalizeVehiclePhotos(parsedValue);
    } catch {
      return [value];
    }
  }
...
function buildVehicleLabel(form) {
  ...
  const vehicleDetails = [form.vehicleColor, form.vehiclePlate]
    .map((item) => item.trim())
    .filter(Boolean);
  return [vehicleName, ...vehicleDetails].filter(Boolean).join(" - ") || form.vehicleLabel.trim();
}
...
function buildProfileForm(profile, fallbackUser) {
  ...
  return {
    ...
    vehiclePhotos: normalizeVehiclePhotos(profile?.vehicle_photos || fallbackVehicle.photos),
...
function validateImageFile(file, label) {
  if (!file.type.startsWith("image/")) return `Choisis une image JPG, PNG ou WebP pour ${label}.`;
  if (file.size > maxImageSize) return `La photo ${label} ne doit pas depasser 5 Mo.`;
  return "";
}
...
export default function EditProfile({ navigate, user }) {
  ...
  useEffect(() => {
    return () => {
      vehiclePreviewUrlsRef.current.forEach((url) => URL.revokeObjectURL(url));
  ...
  const vehiclePhotos = [
    ...
    ...selectedVehiclePhotos.map((item) => ({ src: item.preview, persisted: false })),
  ];
  ...
  function removeProfilePhoto() {
    ...
    setPhotoPreview("");
    setForm((f) => ({ ...f, photoProfil: "" }));
  }

  function handlePickVehiclePhotos() { vehicleInputRef.current?.click(); }

  function handlePhotoChange(event) {
    const file = event.target.files?.[0];
    if (!file) return;
    const err = validateImageFile(file, "la photo de profil");
  ...
  function handleVehiclePhotosChange(event) {
    ...
    const err = files.map((f) => validateImageFile(f, "le vehicule")).find(Boolean);
    if (err) { setFeedback({ message: err, tone: "error" }); return; }
    const available = Math.max(maxVehiclePhotos - form.vehiclePhotos.length - selectedVehiclePhotos.length, 0);
    if (!available) {
      setFeedback({ message: `Tu peux garder jusqu'a ${maxVehiclePhotos} photos vehicule.`, tone: "error" });
      return;
    }
    const accepted = files.slice(0, available).map((f) => {
      const preview = URL.createObjectURL(f);
      vehiclePreviewUrlsRef.current.push(preview);
      return { file: f, preview };
    });
    setSelectedVehiclePhotos((prev) => [...prev, ...accepted]);
    setFeedback(files.length > available
      ? { message: `J'ai garde les ${available} premieres photos.`, tone: "success" }
      : { message: "", tone: "" });
    event.target.value = "";
  }

  function removePersistedVehiclePhoto(idx) {
    setForm((f) => ({ ...f, vehiclePhotos: f.vehiclePhotos.filter((_, i) => i !== idx) }));
  }

  function removeSelectedVehiclePhoto(idx) {
    setSelectedVehiclePhotos((prev) => {
      const item = prev[idx];
      if (item?.preview) {
        URL.revokeObjectURL(item.preview);
        vehiclePreviewUrlsRef.current = vehiclePreviewUrlsRef.current.filter((u) => u !== item.preview);
      }
  ...
  async function handleSave(event) {
    ...
    if (!isConfigured || !session?.user?.id) {
      setFeedback({ message: "Connecte Supabase pour enregistrer.", tone: "error" });
      return;
    }
    try {
      setIsSaving(true);
      isSavingRef.current = true;
      setUploadStep("Preparation...");
      setFeedback({ message: "", tone: "" });

      let photoProfil = form.photoProfil;
      if (selectedPhoto) {
        setUploadStep("Upload de l'avatar...");
        if (profile?.photo_profil) await profileService.deleteProfileAsset(profile.photo_profil, "avatars");
        photoProfil = await profileService.uploadProfilePhoto(selectedPhoto, session.user.id);
      ...
      for (let i = 0; i < selectedVehiclePhotos.length; i++) {
        setUploadStep(`Photo vehicule ${i + 1}/${selectedVehiclePhotos.length}...`);
        const url = await profileService.uploadVehiclePhoto(selectedVehiclePhotos[i].file, session.user.id);
        uploadedVehiclePhotos.push(url);
      }
      const nextVehiclePhotos = [...form.vehiclePhotos, ...uploadedVehiclePhotos].slice(0, maxVehiclePhotos);
      const nextVehicleLabel = buildVehicleLabel({ ...form, vehiclePhotos: nextVehiclePhotos });
      ...
      setUploadStep("Synchronisation...");
      try { await refreshProfile(); } catch (e) { console.warn("Profile sync:", e.message); }

      selectedVehiclePhotos.forEach((item) => URL.revokeObjectURL(item.preview));
      vehiclePreviewUrlsRef.current = [];
      setSelectedPhoto(null);
      setSelectedVehiclePhotos([]);
      setPhotoPreview("");
      setForm((f) => ({ ...f, photoProfil, vehicleLabel: nextVehicleLabel, vehiclePhotos: nextVehiclePhotos }));
      setFeedback({ message: "Profil mis a jour avec succes.", tone: "success" });
      setTimeout(() => navigate("profile"), 1200);
    } catch (error) {
      setFeedback({ message: error.message || "Mise a jour impossible.", tone: "error" });
    } finally {
  ...
  return (
    <div className="screen screen--profile page-enter">
        ...
        <form className="profile-editor-card" onSubmit={handleSave}>
          ...
          <div className="profile-editor-avatar">
            <div
              ...
              onClick={() => avatarSource && setLightboxSrc(avatarSource)}
              style={avatarSource ? { cursor: "zoom-in" } : {}}
            >
              {avatarSource ? (
            ...
            <div className="profile-editor-avatar__actions">
              ...
              {avatarSource && (
                <button className="pf-btn pf-btn--small pf-btn--danger" type="button" onClick={removeProfilePhoto}>
          ...
          <label className="profile-editor-field">
            ...
            <input className="profile-editor-field__control" name="fullName" type="text" value={form.fullName} onChange={updateField} placeholder="Ton nom complet" />
          </label>

          <label className="profile-editor-field">
            <span className="profile-editor-field__label">Email</span>
            <input className="profile-editor-field__control" name="email" type="email" value={form.email} disabled readOnly />
          </label>

          <label className="profile-editor-field">
            <span className="profile-editor-field__label">Téléphone</span>
            <input className="profile-editor-field__control" name="phone" type="tel" value={form.phone} onChange={updateField} placeholder="06 12 34 56 78" />
          </label>

          <label className="profile-editor-field">
            <span className="profile-editor-field__label">Campus</span>
            <input className="profile-editor-field__control" name="campus" type="text" value={form.campus} onChange={updateField} placeholder="Ton campus" />
          </label>

          <label className="profile-editor-field">
            <span className="profile-editor-field__label">Bio</span>
            <textarea className="profile-editor-field__control profile-editor-field__control--textarea" name="bio" value={form.bio} onChange={updateField} placeholder="Quelques mots sur toi..." rows={3} />
          </label>
          ...
          <div className="profile-editor-row">
            <label className="profile-editor-field">
              ...
              <input className="profile-editor-field__control" name="vehicleMake" type="text" value={form.vehicleMake} onChange={updateField} placeholder="Ex: Renault" />
            </label>
            <label className="profile-editor-field">
              <span className="profile-editor-field__label">Modèle</span>
              <input className="profile-editor-field__control" name="vehicleModel" type="text" value={form.vehicleModel} onChange={updateField} placeholder="Ex: Clio" />
            </label>
          </div>
          ...
          <div className="profile-editor-row">
            <label className="profile-editor-field">
              ...
              <input className="profile-editor-field__control" name="vehicleColor" type="text" value={form.vehicleColor} onChange={updateField} placeholder="Ex: Bleu" />
            </label>
            <label className="profile-editor-field">
              <span className="profile-editor-field__label">Plaque</span>
              <input className="profile-editor-field__control" name="vehiclePlate" type="text" value={form.vehiclePlate} onChange={updateField} placeholder="AB-123-CD" />
            </label>
          </div>
          ...
          <div className="profile-editor-row">
            <label className="profile-editor-field">
              ...
              <input className="profile-editor-field__control" name="vehicleSeats" type="number" min="1" max="8" value={form.vehicleSeats} onChange={updateField} placeholder="4" />
            </label>
            <label className="profile-editor-field">
              <span className="profile-editor-field__label">Permis</span>
              <input className="profile-editor-field__control" name="driverLicense" type="text" value={form.driverLicense} onChange={updateField} placeholder="Numéro de permis" />
            </label>
          </div>
          ...
          <div className="profile-editor-field">
            ...
            <input accept="image/*" className="profile-photo-input" multiple ref={vehicleInputRef} type="file" onChange={handleVehiclePhotosChange} />
          </div>
          ...
          <button className="profile-editor-save" type="submit" disabled={isSaving}>
      ...
      {lightboxSrc && (
        <ImageLightbox src={lightboxSrc} alt="Photo agrandie" onClose={() => setLightboxSrc("")} />
      )}
    </div>
  );
}
